...
\begin{itemize}
\item \textbf{Complexity}: The increasing variety of themes, in those ever growing collections, has forced a reconsideration of the way to compare them. The operations required to compute each comparison should be simplified as much as possible.
...
\item \textbf{Lifecycle Management of Topic Models}: To facilitate the exploitation of probabilistic topic models, we propose a unified form to publish and (re)use topic models as Web services that are available from online repositories.
